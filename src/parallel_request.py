...
async def process_api_requests_from_file(
    ...
    with open(requests_filepath) as file:
        ...
        if status_tracker.num_tasks_failed > 0:
            logging.warning(f"{status_tracker.num_tasks_failed} / {status_tracker.num_tasks_started} requests failed. Errors logged to {log_save_filepath}.")
    ...
    async def call_api(
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                append_to_jsonl([self.request_json, self.result], log_save_filepath)
                status_tracker.num_tasks_in_progress -= 1
                status_tracker.num_tasks_failed += 1
        else:
            append_to_jsonl([dict(task_id=self.task_id), self.request_json, response], save_filepath)
            status_tracker.num_tasks_in_progress -= 1
...
def parallel_request_openai(
    ...
    if log_save_filepath is None:
        log_save_filepath = requests_filepath.replace(".jsonl", "_logs.jsonl")
